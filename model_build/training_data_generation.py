...
data = pd.DataFrame({
    ...
    #'location': np.random.choice(['USA', 'Canada', 'UK', 'Germany', 'France', 'India', 'China'], num_samples),
    'account_age': np.random.randint(1, 3650, num_samples),  # Account age in days (up to 10 years)
    #'transaction_method': np.random.choice(['CreditCard', 'BankTransfer', 'PayPal', 'Cryptocurrency'], num_samples),
...
# Add some patterns to fraudulent cases
...
# Convert categorical variables to dummy/one-hot encoding for machine learning
#data = pd.get_dummies(data, columns=['location', 'transaction_method'], drop_first=True)

# One-hot encode categorical variables
#df_encoded = pd.get_dummies(data, columns=['transaction_method', 'location'])
